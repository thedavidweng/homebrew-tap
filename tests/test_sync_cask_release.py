...
def sample_tg_drive_cli_payload(version="0.2.0"):
    return {
        "tag_name": f"v{version}",
        "assets": [
            {
                "name": "td_darwin_universal.tar.gz",
                "digest": "sha256:63b33591e7097a8e323a56c6f67fdd03c528bf1b52f34a47ee7c2c93bea2f842",
            }
        ],
    }
...
class SyncPixivSwiftUIReleaseTests(unittest.TestCase):
    ...
    def test_main_supports_tg_drive_cli_app(self):
        module = load_module()
        called_apps = []
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def fetch_release(app):
                called_apps.append(app)
                return sample_tg_drive_cli_payload()
